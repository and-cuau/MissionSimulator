...
module.exports = (missionId, mission_start_time, missionObjectives) => {
  let objectiveJobs = [];
  let objectiveid = 1;
  ...
  console.log("now information: "+ now);

  console.log("timezone offset: " +(now.getTimezoneOffset()));

  const delay = mission_start_time - now;
  objectiveJobs[0].opts.delay = delay;
