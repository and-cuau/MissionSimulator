...
app.use(
  cors({
    origin: process.env.CLIENT_URL || "*", // Replace with actual frontend url when needed
    credentials: true,
  }),
);
...
queueEvents.on("completed", async ({ jobId }) => {
  // Parent job finished -> create child job
...
(async () => {
  ...
  // start server after setting up tables
...
app.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      const result = await pool.query(
        `SELECT start_time FROM missions WHERE id = $1`,
        [missionId],
      );
      const start_time = result.rows[0].start_time;

      addMissionToFlow(missionId, start_time, objectives);
      initiateMission();
      initiateObjective();
...
app.get("/missions/search", async (req, res) => {
  console.log(`GET /missions/search was called`);
  const query = req.query.q || "";
  try {
    const result = await pool.query(
      "SELECT * FROM missions WHERE mission_title ILIKE $1",
      [`%${query}%`],
    );
...
app.delete(
  ...
  logAction("DELETED MISSION", (req) => req.missionId),
);
